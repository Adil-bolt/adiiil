  ...
  const stats = useMemo(() => {
    const relevantAppointments = appointments.filter(apt => {
      ...
      const isInRange = selectionRange 
        ? isWithinInterval(aptDate, selectionRange)
        : isSameDay(aptDate, selectedDate);
      // Exclure les pauses déjeuner
      return isInRange && !apt.isLunchBreak;
    });
    ...
    const typeCount = relevantAppointments.reduce((acc, apt) => {
      if (apt.isCanceled) {
        ...
        // Ne vérifier que isNewPatient pour les nouveaux patients
        if (apt.isNewPatient) {
          acc['NOUVELLE CONSULTATION'] = (acc['NOUVELLE CONSULTATION'] || 0) + 1;
        } else if (apt.isGratuite || apt.type?.toUpperCase() === 'GRATUIT') {
          acc['GRATUIT'] = (acc['GRATUIT'] || 0) + 1;
        } else {
          acc['SUIVI'] = (acc['SUIVI'] || 0) + 1;
        }
      }
      return acc;
        ...
        <div className="col-span-7 grid grid-cols-7">
          {days.map((day, dayIndex) => (
            <button
              ...
              className={`
                w-8 h-7 text-sm relative flex items-center justify-center
                ${!isSameMonth(day, currentMonth) ? 'text-gray-400' : 'text-gray-900'}
                ${isDateInRange(day) ? 'bg-indigo-100 relative z-0' : ''}
                ${selectionRange && isSameDay(day, selectionRange.start) ? 'rounded-l-full' : ''}
                ${selectionRange && isSameDay(day, selectionRange.end) ? 'rounded-r-full' : ''}
            ...
            >
              <span className={`
                flex items-center justify-center w-7 h-7 rounded-full relative
                ${isSameDay(day, selectedDate) ? 'bg-indigo-600 text-white font-semibold z-20' : 'z-10'}
              `}>
                {format(day, 'd')}
              </span>
              {hasAppointmentsOnDate(day) && !isSameDay(day, selectedDate) && (
                <span className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-indigo-400 rounded-full z-30"></span>
              )}
            </button>
          ))}
        </div>
